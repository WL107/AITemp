...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    db = InfluxDBClient('192.168.1.1', 8086, 'test', 'password', 'testdb')
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if names[c] == "person": #person
                            ...
                            #雙鏡頭座標校正                          
                            arr=arr[max(120-int((xyxy[3]+20)/4),0):max(120-int((xyxy[1]+20)/4),0),max(160-int((xyxy[2]-0)/4),0):max(160-int((xyxy[0]-0)/4),0)]

                            if arr.size !=0:
                                
                                annotator.box_label(xyxy," TEMP" +str(centikelvin_to_celsius(arr.max())), color=colors(c, True))
                                print(float(centikelvin_to_celsius(arr.max())))
                                
                                if tempmax <= float(centikelvin_to_celsius(arr.max())):
                                    print(tempmax)
                                    tempmax = float(centikelvin_to_celsius(arr.max()))
                            ...
                            influx_metric = [{
                                "measurement": "temp",
                                "fields": {
                                    "TEMP": float(centikelvin_to_celsius(arr.max())),
                                }
                            }]
                            db.write_points(influx_metric)
                            if float(centikelvin_to_celsius(arr.max()))>=37.5:
                                save_one_box((0,0,640,480), im0, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=True)
            if int(time.time()/60)==timeloop:
                influx_metric = [{
                    "measurement": "1m_tempmax",
...
def centikelvin_to_celsius(t):
    Tmp = (t - 27315) / 100
    if Tmp>=36 and Tmp<40:
        return Tmp
    elif Tmp<36 and Tmp>=30:
        minus = int(Tmp)
        return 36- minus + Tmp
    else:
        return float(0)
